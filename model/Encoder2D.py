...
# A correct version?
    def forward(self, x):
        ...
        x_3 = x_3.view(n, t, out_c, new_h, new_w).permute(0, 3, 4, 2, 1)  # Reorder to [N, H, W, O, T]
        x_3 = x_3.reshape(-1, out_c, t)  # Viewing as required for 1D conv [N*O*H*W, Out_C, T]
        ...
        x_5 = x_5.view(n, t, out_c, new_h, new_w).permute(0, 3, 4, 2, 1)  # Reorder to [N, H, W, O, T]
        
        x_5 = x_5.reshape(-1, out_c, t)  # Viewing as required for 1D conv [N*O*H*W, Out_C, T]
        x_5 = self.relu(self.bn_t_5(self.t_5(x_5)))  # Apply 1D conv across temporal

        # Element-wise addition of outputs from both paths
        x = x_3 + x_5

        x = x.view(n, new_h, new_w, out_c, t).permute(0, 4, 3, 1, 2)  # Return back to [N, T, C, H, W]
